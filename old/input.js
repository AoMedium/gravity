	...
	function(e) {
		...
		if (disableKeyInput) {
			...
			switch (key) {
				...
				case 187: //+
					viewScale = round(viewScaleMult * viewScale, 6);
				break;
				case 189: //-
					if (viewScale / viewScaleMult > 0.000001) {
						viewScale = round(viewScale / viewScaleMult, 6);
					}
				break;
				

				case 48: //0
					viewScaleMult = round(viewScaleMult + viewScaleMultStep, 6);
				break;
				case 57: //9
	...
	switch (cmdSplit[0]) {
		case "system":
			switch (cmdSplit[1]) {
				...
				case "generate":
					generateSystem(cmdSplit[2], cmdSplit[3], cmdSplit[4], cmdSplit[5]);
					updateCmdResult("attempted to generate system with " + cmdSplit[2] + " radius and " + cmdSplit[3] + " entities");
				break;
				...
				case "save":
					preserveCmdBox = true;
					cmdBox.value = saveSystem();
					updateCmdResult("outputted saved system");
				break;
			...
			switch (cmdSplit[1]) {
				case "set":
					...
					switch (cmdSplit[2]) {
						...
						case "type":
						case "disguise":
							var val = cmdSplit[3].replace("_", " ");
							for (var i = 0; i < typeOfEntity.length; i++) {
								if (typeOfEntity[i].typeName == val) {
									console.log(i)
									roidDisguise = i;
		...
		case "set":
			...
			switch (cmdSplit[1]) {
				...
				case "trailsamples":
					trailSamples = parseInt(val);
					updateCmdResult("set trail samples to " + val);
				break;
				...
				case "radiusconst":
					radiusConst = parseFloat(val);
					updateCmdResult("set radiusConst to " + val);
				break;
		...
		case "toggle":
			switch (cmdSplit[1]) {
				...
				case "usebackground":
					...
					toggleBackground();
					updateCmdResult("toggled useBackground to " + useBackground);
				break;
		...
		case "display":
			switch (cmdSplit[1]) {
				case "set":
					switch (cmdSplit[2]) {
						case "fontsize":
							var text = parseInt(cmdSplit[3]);
							var size = parseInt(cmdSplit[4]);

							fontSizes[text - 1] = size;
							updateCmdResult("set fontsize of textFont " + cmdSplit[3] + " to " + cmdSplit[4]);
						break;
					}
				break;
				case "mode":
...
function saveSystem() {
	...
	for (var i = 0; i < entities.length; i++) {
		saveString += entities[i].id + splitData + 
					entities[i].mass + splitData + 
					entities[i].x + "," + entities[i].y + splitData +
					entities[i].dx + "," + entities[i].dy + splitData +
					entities[i].fixed + splitData +
					entities[i].type.typeName + splitData +
					entities[i].typeFixed + splitData +
					entities[i].isPrimary + splitData +
					entities[i].surfaceColor + "," + entities[i].atmosphereColor

		saveString += splitEntity;
	}

	return saveString;
}

function importSystem(systemString) {
