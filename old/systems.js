...
var starColor = ["255, 182, 108","255, 218, 181", "255, 237, 227","249, 245, 255","213, 224, 255", "162, 191, 255","146, 181, 255"];
...
{ //EntityType Data
	...
	var star = new EntityType("star", 1000000, 1.4);
		star.draw = function(sX, sY, r, color) {
			// if (r * viewScale < 1) {
			// 	star.drawPersistGradient(sX, sY, 0.2, [1,r], [[0,color,1],[0.1,color,0.2],[1,color,0]]);
			...
			// } else {
				...
				star.drawGradient(sX, sY, r, [1, 20], [[0,color,0.4],[0.02,color,0.2],[1,color,0]]);
				star.drawBody(sX, sY, r, color);
				star.drawGradient(sX, sY, r, [0.4, 1], [[0, "255,255,255", 0.8],[0.8,color,1]]);
...
function EntityType(typeName, massThreshold, density) {
	...
	this.drawPersistGradient = function(sX, sY, r, rArray, grdArray) {
		var grd = c.createRadialGradient(sX, sY, r * rArray[0], sX, sY, r * rArray[1]);
		...
		c.fillStyle = grd;
		c.beginPath();
		c.arc(sX, sY, r * rArray[1], 0, 2 * Math.PI);
		c.fill();
		c.closePath();
	}

	this.drawAtmosphere = function(sX, sY, r, thickness, color = "145, 224, 255") {
		var grd = c.createRadialGradient(sX, sY, r * viewScale, sX, sY, (r + thickness * radiusConst) * viewScale);
		...
		c.fillStyle = grd;
		c.beginPath();
		c.arc(sX, sY, (r + thickness * radiusConst) * viewScale, 0, Math.PI * 2);
		c.fill();
		c.closePath();
	}
	...
	this.drawShading = function(sX, sY, r, angleInfo, thickness = 1, hasAtmosphere = false) {
		...
		var shadeColor = "0,0,0";

		if (angleInfo == "none") {
			return;
		} else if (angleInfo == undefined) {
			var grd = c.createRadialGradient(sX, sY, 0, sX, sY, r * viewScale);
			grd.addColorStop(0.3, "rgba("+ shadeColor + ",0)");
			grd.addColorStop(1, "rgba("+ shadeColor + ",0.4)");
		} else {
			...
			grd.addColorStop(0, "rgba("+ shadeColor + "," + angleInfo[2] / 200000 + ")"); //(angleInfo[2] / 200000)
			grd.addColorStop(0.55, "rgba("+ shadeColor + ",0.95)");
		}
	...
	this.drawTail = function(sX, sY, r, angleInfo, moveInfo, gasColor) {
		if (angleInfo == "none" || angleInfo == undefined) {
			return;
		}
...
var sol_alpha = new EntitySystem("sol_alpha")
	...
	sol_alpha.initSystemObjects = function() {
		...
		var Terra = new PrimaryObject("o", "Terra", 1000, AU);
			...
			var Muna = new PrimaryObject("o", "Muna", 12, AU * 0.002569);
				Muna.center = "Terra";
...
function EntitySystem(systemName = "untitled_system") {
	...
	this.coloringMode = "colormode:sequential"; //coloring mode
			...
			for (var i = 0; i < this.entities.length; i++) {
				...
				switch (this.entities[i].motionType) {
					...
					case "o":
						...
						//console.log(this.entities[i].id + " " + this.entities[i].isClockwise)
					break;
				}
