...
var textColor, textColor2, outlineColor, outlineColor2, fillColor, auraColor;
...
function drawGUI() {
	if (!hideUI) {
		...
		c.fillText("selected system: " + systems[selSystem].systemName, 20, 3 * guiHeight + guiOffset);
		...
		c.fillText("offset step: " + offset.val, 20, 10 * guiHeight + guiOffset);
		...
		c.fillText("[`]    using nightMode: " + nightMode, 20, 20 * guiHeight + guiOffset + 200);
		c.fillText("[h]    hide UI", 20, 21 * guiHeight + guiOffset + 200);
		c.fillText("[c]    cast orbits", 20, 22 * guiHeight + guiOffset + 200);
...
function spawnRoid(isUser, x, y) {
	if (isUser) {
		...
		if (orbitTarget && isTargetting) { //&& target != undefined
			...
			createNewOrbit("ID" + idCount, roidMass.val, sepVal[2], angle, target);
		} else {
...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, isPrimary = false, primaryColor) {
	...
	this.draw = function() {
		...
		if (scaleX > innerWidth || scaleY > innerHeight || scaleX < 0 || scaleY < 0) {
			...
			if (!stopClear) {
				...
				//console.log(auraColor + " " + outlineColor2);
