...
// 	["spawn range", simSystem.spawnRange, 2],
// 	["cull range", simSystem.spawnRange * 2, 2],
// 	["isPaused", isPaused, 3]
// ];
...
function drawGUI() {
	...
	c.fillText("cull range: " + simSystem.spawnRange * 2, 20, 3 * guiHeight + guiOffset + 100);

	c.fillText("[s]    spawn at click: " + spawnAtClick, 20, 5 * guiHeight + guiOffset + 100);
	c.fillText("[z][x] roid disguise: " + typeOfEntity[roidDisguise], 20, 6 * guiHeight + guiOffset + 100);
	...
	c.fillText("[t][f] target: " + target, 20, 12 * guiHeight + guiOffset + 100);
	c.fillText("[n][m] time step: " + timeStep + "x", 20, 13 * guiHeight + guiOffset + 100);
	c.fillText("[p]    isPaused: " + isPaused, 20, 14 * guiHeight + guiOffset + 100);
	c.closePath();
...
function spawnRoid(isUser, x, y) {
	if (isUser) {
		...
		idCount++;
	} else if (entities.length < simSystem.entityCap) {
...
function cullOutside() {
	...
	for (var i = 0; i < entities.length; i++) {
		...
		if (entities[i].x < originX - simSystem.spawnRange * 2 || entities[i].x > originX + simSystem.spawnRange * 2 || entities[i].y < originY - simSystem.spawnRange * 2 || entities[i].y > originY + simSystem.spawnRange * 2) {
