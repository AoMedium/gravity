...
{ //EntityType Data
	var dust = new EntityType("dust", 0, 5, 0);
	...
	var roid = new EntityType("roid", 10, 2, 0);
		roid.traceDetail = function() {
			//console.log("roid");
		}

	var dwarf_planet = new EntityType("dwarf_planet", 50, 4, 0.65);
		dwarf_planet.traceAura = function(sX, sY, r) {
			drawAura(sX, sY, r, dwarf_planet.luminance, r + 0.5);
		}

	var terrestrial = new EntityType("terrestrial", 500, 5, 0.8);
		terrestrial.traceAura = function(sX, sY, r) {
			drawAura(sX, sY, r, terrestrial.luminance, r + 0.4);
			drawAura(sX, sY, r, terrestrial.luminance, r + 0.8);
			drawAura(sX, sY, r, terrestrial.luminance, r + 1);
		}


	var gas_giant = new EntityType("gas_giant", 10000, 1, 0);
	...
	var brown_dwarf = new EntityType("brown_dwarf", 500000, 1.4, 0.8);
		...
		brown_dwarf.traceAura = function(sX, sY, r) {
			drawAura(sX, sY, r, brown_dwarf.luminance, r + r / 4);
		}

	var star = new EntityType("star", 1000000, 1.4, 0.8);
		star.traceAura = function(sX, sY, r) {
			drawAura(sX, sY, r, star.luminance, 1.2 * r);
			drawAura(sX, sY, r, star.luminance, 1.5 * r);
			drawAura(sX, sY, r, star.luminance, 2 * r);
			drawAura(sX, sY, r, star.luminance, 3 * r);
			drawAura(sX, sY, r, star.luminance, 4.5 * r);
		}


	var negative_matter = new EntityType("negative_matter", 900000000, 100, 1);
		negative_matter.traceAura = function(sX, sY, r) {
			drawAura(sX, sY, r, negative_matter.luminance, r + r / 2);
			drawAura(sX, sY, r, negative_matter.luminance, r + r * 2.5,);
		}
}
...
function drawAura(sX, sY, r, luminance, rDraw) {
	var opacity = round(r / (rDraw * 6), 2) * luminance;
	...
	c.beginPath();
	c.arc(sX, sY, rDraw * viewScale.val, 0, 2 * Math.PI);
...
function EntityType(typeName, massThreshold, density, luminance = 1) {
...
var systems = [sol_alpha, sol_duo, orbit_sim, owo_system, cannon_system, test_system, proto_sol, isolated_system];
			...
			for (var i = 0; i < this.entities.length; i++) {
				...
				switch (this.entities[i].motionType) {
					...
					case "o":
						...
						if (this.entities[i].center != undefined) { //is a moon
