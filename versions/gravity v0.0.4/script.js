...
function simulateSystem(id) {
	switch (id) {
		case "sol_alpha":
			createNew("Sol Alpha", 0, 0, 1000, 0, 0, false);
			createNew("Kas", 0, 40, 20, 158, 0, false);
			createNew("Ayca", 0, 120, 50, 91, 0, false);
			createNew("Terra", 0, -270, 70, -62, 0, false);
			createNew("Muna", 0, -290, 11, 0, 0, false);
			createNew("Jaen", 30, 500, 20, 45, 0, false);
			createNew("Muerin", -700, 0, 200, 0, 38, false);
			createNew("Hera", -730, -40, 12, 45, 0, false);
			createNew("Iomus", 900, 0, 400, 0, -33, false);
		break;
		case "sol_beta":
		break;
		case "cannon_system":
			createNew("cannon1", 0, -100, 10000, 0, 0, true);
			createNew("cannon2", 0, 100, 10000, 0, 0, true);
			createNew("cannon3", 0, -25, 1000, 0, 0, true);
			createNew("cannon4", 0, 25, 1000, 0, 0, true);
			createNew("ball", -500, 0, 12, 0, 0, false);
		break;
	}
}
...
function drawGUI() {
	...
	c.beginPath();
	c.font = "12px Arial";
	c.fillText("system: " + system, 20, 30);
	c.fillText("zoom scale: " + scale, 20, 45);
	c.fillText("zoom step: " + offsetStep, 20, 60);
	c.fillText("entities: " + entities.length, 20, 75);
	c.closePath();
...
function calcVector(x, y) {
	var theta, vecx, vecy;
	if (x == 0) {
		x += 0.0000001; //since y/x for x = 0 is still compiled, add 0.0001 to ensure not 1/0
		theta = Math.PI / 2;
	} else {
		...
		if (this.mass > 10 || showTrivial) {
			...
			if (infoMode == 0) {
				...
				c.fillText(Math.round(this.mass) + " / (" + 
					Math.round(this.x - originX) + ", " + 
					Math.round(this.y - originY) + ")", scaleX + 10, scaleY + 20);
			}
