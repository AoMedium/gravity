	...
	function(e) {
		...
		if (disableKeyInput) {
			switch (key) {
				...
				case 38: //up
					if (cmdBox.value == "") {// probably better if not previous cmds
						cmdBox.value = cmdHistory[cmdHistory.length - 1]; //latest cmd
						if (iCmd > 0) {
							iCmd--;
						}
			...
			if (isTargetting == false) {
				switch (key) {
					case 38:
						originY -= stepModeArray[1].val / viewScale.val; //using as in v0.0.2
					break;
					case 40:
						originY += stepModeArray[1].val / viewScale.val;
					break;
					case 37:
						originX -= stepModeArray[1].val / viewScale.val;
					break;
					case 39:
						originX += stepModeArray[1].val / viewScale.val;
					break;
	...
	switch (cmdSplit[0]) {
		...
		case "toggle":
			switch (cmdSplit[1]) {
				...
				case "radiuscull":
					radiusCull = !radiusCull;
					updateCmdResult("toggled radiusCull to " + radiusCull);
				break;
