...
var renderCulling = true; //barely any difference
	...
	this.draw = function() {
			...
			) 
		{
			...
			if (!stopClear) {
						...
						case 0:
							...
							for (var i = 1; i < this.lastPos.length; i++) {
								...
								if (showTrailNodes) { //not sure if having condition induces additional load
									...
									//c.arc(viewScale.val * (this.lastPos[i][0] - originX) + ptrX, viewScale.val * (this.lastPos[i][1] - originY) + ptrY, 0.5, 0, Math.PI * 2)
									// if (i == this.lastPos.length - 1) {
									// 	trailNodeRadius = 4;
									// } else if (i == this.lastPos.length - 2) {
									// 	trailNodeRadius = 2;
									// }
									c.fillRect(viewScale.val * (this.lastPos[i][0] - originX) + ptrX - trailNodeRadius, viewScale.val * (this.lastPos[i][1] - originY) + ptrY - trailNodeRadius, trailNodeRadius, trailNodeRadius);
									trailNodeRadius = 1;
								}
							}
						break;
				...
				if (this.radius * viewScale.val < 0.5 && (this.isPrimary || target.id == this.id)) {
					c.beginPath();
