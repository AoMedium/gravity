...
{ //EntityType Data
	...
	var star = new EntityType("star", 1000000, 1.4, 6);
		...
		star.traceAura = function(sX, sY, r) {
			star.drawAura(sX, sY, r, [1.2 * r, 1.5 * r, 2 * r, 3 * r, 4.5 * r]);
		}
		// star.drawBody = function(sX, sY, r) {
		// 	//console.log(star.properties.temperature)
		// }
...
function EntityType(typeName, massThreshold, density, scattering = 1) {
	...
	this.auraColor = "rgba(255,255,255"//auraColor;

	this.traceDetail = function() {}
	...
	this.drawAura = function(sX, sY, r, rDrawArray) {
		c.strokeStyle = this.auraColor + ",1)";

		for (var i = 0; i < rDrawArray.length; i++) {
			var opacity = round(r / (rDrawArray[i] * this.scattering), 2);
			c.fillStyle = this.auraColor + "," + opacity + ")";
	...
	this.drawBody = function(sX, sY, r) {
...
var sol_alpha = new EntitySystem("sol_alpha")
	...
	sol_alpha.initSystemObjects = function() {
		...
		var Terra = new PrimaryObject("o", "Terra", 1000, AU);
			...
			var Craft = new PrimaryObject("o", "Craft", 10, 20);
				Craft.center = "Terra";
...
var test_system = new EntitySystem("test_system");
	...
	test_system.initSystemObjects = function() {
		...
		var Test_Planet = new PrimaryObject("o", "Test Planet", 100, 60);
