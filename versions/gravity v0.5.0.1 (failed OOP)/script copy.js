...
function createNew(id, mass, x, y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor) {
	entities.push(new Entity(id, mass, x, y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor));
}
...
function GravityCore(id, mass, x, y, dx, dy) {
...
function SystemObject(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, typeFixed = false, isPrimary = false, primaryColor) {
	...
	this.gCore = new GravityCore(this.id);
	...
	this.update = function() {
		this.gCore.update(this.mass, this.x, this.y, this.dx, this.dy, this.radius);

		this.mass = this.gCore.mass;
		this.x = this.gCore.x;
		this.y = this.gCore.y;
		this.dx = this.gCore.dx;
		this.dy = this.gCore.dy;
