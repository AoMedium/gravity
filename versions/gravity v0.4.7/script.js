...
function createNew(id, mass, x, y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor) {
	entities.push(new Entity(id, mass, midX + x, midY + y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor));
}
	...
	if (center == undefined) {
		...
		for (var i = 0; i < entities.length; i++) {
			...
			if (entities[i].id == center) {
				...
				this.centerX = entities[i].x - midX; //somehow needs to include midX here.....
				this.centerY = entities[i].y - midY;
...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, typeFixed = false, isPrimary = false, primaryColor) {
	...
	this.update = function() {
		...
		if (isPredictPath && target.id == this.id) {
			this.predictPos = [];
	...
	this.isInRender = function() {
		if (this.renderX - this.radius * viewScale.val > innerWidth || 
	...
	this.draw = function() {
		...
		this.renderX = (viewScale.val * (this.x - ptrX)) + midX; //find distance from origin, viewScale.val, then render from pointer
		this.renderY = (viewScale.val * (this.y - ptrY)) + midY;
		...
		if (
			...
			(viewScale.val * (this.lastPos[0][0] - ptrX) + midX > innerWidth ||
			viewScale.val * (this.lastPos[0][1] - ptrY) + midY > innerHeight ||
			viewScale.val * (this.lastPos[0][0] - ptrX) + midX < 0 ||
			viewScale.val * (this.lastPos[0][1] - ptrY) + midY < 0) &&
			...
			if (!stopClear) {
					...
					switch (trailMode) {
						case 0:
							...
							for (var i = 0; i < this.lastPos.length; i++) {
								c.lineTo(viewScale.val * (this.lastPos[i][0] - ptrX) + midX, viewScale.val * (this.lastPos[i][1] - ptrY) + midY);
								...
								if (showTrailNodes) { //not sure if having condition induces additional load
									...
									c.fillRect(viewScale.val * (this.lastPos[i][0] - ptrX) + midX - trailNodeRadius / 2, viewScale.val * (this.lastPos[i][1] - ptrY) + midY - trailNodeRadius / 2, trailNodeRadius, trailNodeRadius);
									bold = !bold
								}
							}
						break;
						case 1:
							for (var i = 0; i < this.lastPos.length; i++) {
								c.lineTo(viewScale.val * (this.lastPos[i][0]) + midX, viewScale.val * (this.lastPos[i][1]) + midY)
							}
						break
				...
				if (isPredictPath && target.id == this.id) {
					...
					for (var i = 0; i < this.predictPos.length; i++) {
						c.lineTo(viewScale.val * (this.predictPos[i][0] - ptrX) + midX, viewScale.val * (this.predictPos[i][1] - ptrY) + midY)
						
						//c.fillRect(viewScale.val * (this.predictPos[i][0] - ptrX) + midX - trailNodeRadius / 2, viewScale.val * (this.predictPos[i][1] - ptrY) + midY - trailNodeRadius / 2, trailNodeRadius, trailNodeRadius)
					}
				...
				if (this.radius * viewScale.val < 0.5 && (this.isPrimary || target.id == this.id) && radiusColoring) {
					c.beginPath();
				...
				if (this.mass >= trivialThreshold || showTrivial || this.id == target.id || this.isPrimary) {
					...
					if (infoMode > 2) { //name, type and mass
						c.fillText(this.type.typeName + " / " + Math.round(this.mass), tagX + tagXOffset, tagY + 20); //midX is effectively the origin, rather than the screen center
					}

					if (infoMode > 3) { //name, type, mass and vals  || target.id == this.id
						c.font = textFont3;
						c.fillText("(" + Math.round(this.x - midX) + ", " + Math.round(this.y - midY) + ")", tagX + tagXOffset, tagY + 32);
	...
	this.pushNewPos = function() {
		switch (trailMode) {
			...
			break;
			case 1:
				this.lastPos.push([this.x - ptrX, this.y - ptrY]);
			break;
		}
