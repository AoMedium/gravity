...
var centreX, centreY, centreMass, centreDX, centreDY;
...
function updateNightMode() {
	if (nightMode) {
		...
		outlineColor2 = "#eee"; //rgba(0,0,0,0.5)
		auraColor = "#999"; //rgba(0,0,0,0.3)
		...
		outlineColor2 = "rgba(0,0,0,0.5)"; //rgba(0,0,0,0.5)
		auraColor = "rgba(0,0,0,0.3)"; //rgba(0,0,0,0.3)
	}
}
function resetStyle() {
...
function createNewOrbit(id, mass, radius, angle, centre, isClockwise, type, isPrimary) { //WIP
	var x, y, dx, dy, v, vx, vy;

	if (centre == undefined) {
		// console.log("undefined for " + id + ", using x" + simSystem.centreX + " y" + simSystem.centreY + " from " + simSystem.centre)
		this.centreX = simSystem.centreX;
		this.centreY = simSystem.centreY;
		this.centreMass = simSystem.centreMass;
		this.centreDX = simSystem.centreDX// / vScale;
		this.centreDY = simSystem.centreDY// / vScale;
	} else { //finding the centre
		for (var i = 0; i < simSystem.freeEntities.length; i++) {
			// console.log(simSystem.freeEntities[i][0]);
			if (simSystem.freeEntities[i][0] == centre) {
				this.centre = simSystem.freeEntities[i][0]; //testing for found
				this.centreMass = simSystem.freeEntities[i][1];
				this.centreX = simSystem.freeEntities[i][2];
				this.centreY = simSystem.freeEntities[i][3];
				this.centreDX = simSystem.freeEntities[i][4] / vScale;
				this.centreDY = simSystem.freeEntities[i][5] / vScale;
				
				// console.log("break in 1 for " + id + centre);
				break;
			}
		}
		// console.log("next x" + this.centreX + " y" + this.centreY);
		if (this.centre == undefined) {
			for (var i = 0; i < entities.length; i++) {
				if (entities[i].id == centre) {
					//console.log(entities[i].id + " x" + entities[i].x + " y" + entities[i].y);
					this.centreX = entities[i].x - ptrX; //somehow needs to include ptrX here.....
					this.centreY = entities[i].y - ptrY;
					this.centreMass = entities[i].mass;
					this.centreDX = entities[i].dx / vScale;
					this.centreDY = entities[i].dy / vScale;
					//console.log("break in 2 for " + id + centre);
					break;
				}
			}
		}
		//console.log("next 2x" + this.centreDX + " y" + this.centreDY);
	}

	x = Math.round(radius * Math.cos(angle) + this.centreX);
	y = Math.round(radius * Math.sin(angle) + this.centreY);
	

	var sepVal = calcSep(x, y, this.centreX, this.centreY);

	v = Math.sqrt(G * this.centreMass / sepVal[2]) / vScale;
	...
	//simply adding dx works linearly only

	createNew(id, mass, x, y, dx + this.centreDX, dy + this.centreDY, false, type, isPrimary);
	//console.log("dx" + (dx + this.centreDX) + " dy" + (dy + this.centreDY));
...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, isPrimary = false) {
	...
	if (isPrimary) {
		this.trailColour = colours[Math.round(Math.random() * (colours.length - 1))];
	} else {
		this.trailColour = outlineColor2;
	}
	...
	this.periodicUpdate = function() {
		if (simSystem.centre == this.id) {
			simSystem.centreX = this.x - originX;
			simSystem.centreY = this.y - originY;
			simSystem.centreMass = this.mass;
			simSystem.centreDX = this.dx;
			simSystem.centreDY = this.dy;
		}
	}
