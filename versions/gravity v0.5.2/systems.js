...
{ //EntityType Data
	...
	var star = new EntityType("star", 1000000, 1.4, 6);
		star.traceAura = function(sX, sY, r, color) {
			star.drawAura(sX, sY, r, [1.2 * r, 1.5 * r, 2 * r, 3 * r, 4.5 * r], color);
		}
...
function EntityType(typeName, massThreshold, density, scattering = 1) {
	...
	this.drawOuterGradient = function(sX, sY, r, color) {
		...
		grd.addColorStop(0.08, color + ",0.15)");
	...
	this.drawInnerGradient = function(sX, sY, r, color) {
		var grd = c.createRadialGradient(sX, sY, r * viewScale * 0.4, sX, sY, r * viewScale * 0.85);
