...
function createNew(id, mass, x, y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor, temperature) {
	entities.push(new Entity(id, mass, x, y, dx, dy, fixed, type, typeFixed, isPrimary, primaryColor, temperature));
	//console.log(id + " " + x)
}
function createNewOrbit(id, mass, radius, angle, center, isClockwise, type, typeFixed, isPrimary, primaryColor, temperature) { //WIP
	var x, y, dx, dy, v;
	...
	createNew(id, mass, x, y, dx + this.centerDX, dy + this.centerDY, false, type, typeFixed, isPrimary, primaryColor, temperature);
...
function spawnRoid(isUser, x, y) {
	if (isUser) {
		...
		if (orbitTarget) { //&& target != undefined
			var orbitPara = calcOrbitPara(target.x, target.y, x, y);
...
function calcOrbitPara(x1, y1, x2, y2) {
...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, typeFixed = false, isPrimary = false, primaryColor, temperature) {
			...
			) 
		{
			...
			if (!stopClear) {
				...
				if (this.radius * viewScale < 0.5 && (this.isPrimary || target.id == this.id) && radiusColoring) {
					...
					if (this.type == star) {
						var color = this.properties.getColor();
						//this.type.traceAura(this.renderX, this.renderY, this.radius, color);
						this.type.drawOuterGradient(this.renderX, this.renderY, this.radius, color);
						this.type.drawInnerGradient(this.renderX, this.renderY, this.radius, color);
						//this.type.drawBody(this.renderX, this.renderY, this.radius, color);
					} else {
					...
					// if (this.id == "Verza") {
					// 	c.beginPath();
					// 	c.lineWidth = 3200 * viewScale;
					// 	c.strokeStyle = "rgba(200,200,200, 0.1)"
					// 	c.arc(this.renderX, this.renderY, this.radius * 55 * viewScale, 0, Math.PI * 2);
					// 	c.stroke();
					// }
