...
{ //EntityType Data
	...
	var terrestrial = new EntityType("terrestrial", 500, 5);
		terrestrial.traceAura = function(sX, sY, r) {
			c.arc(sX , sY, (r + 1) * viewScale.val, 0, 2 * Math.PI);
			c.stroke();
			c.beginPath();
	...
	var star = new EntityType("star", 1000000, 1.4);
		star.traceAura = function(sX, sY, r) {
			c.arc(sX , sY, (1.1 * r) * viewScale.val, 0, 2 * Math.PI);
			c.stroke();
			c.beginPath();
			c.arc(sX , sY, (1.4 * r) * viewScale.val, 0, 2 * Math.PI);
			c.stroke();
			c.beginPath();
			c.arc(sX , sY, (2.2 * r) * viewScale.val, 0, 2 * Math.PI);
		}
