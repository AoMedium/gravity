...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, typeFixed = false, isPrimary = false, primaryColor, planetColor = planetColors[Math.round(Math.random() * (planetColors.length - 1))], temperature) {
	...
	this.draw = function() {
			...
			) 
		{
			...
			if (!stopClear) {
				...
				if (this.radius * viewScale < 0.5 && radiusColoring) {
					...
					if (isUsingShading) {
						if (!foundStar) {
							this.type.draw(this.renderX, this.renderY, this.radius, undefined, color);
						} else {
							if (iClosestStar > entities.length - 1 || entities[iClosestStar].type != star) { //in the event of a star being deleted
								...
								var angleInfo = [vector[0], vector[1], sepVal[2]];

								this.type.draw(this.renderX, this.renderY, this.radius, angleInfo, color);
							}
						}
					} else {
						this.type.draw(this.renderX, this.renderY, this.radius, "none", color);
					}
