...
var offsetStep = new StepVariable("offsetStep", 10, 2, 0.2);
...
//var trackPosTime = setInterval(trackPos, 500); //balance between max trail to improve performance
var maxTrail = 200;
var showTrails = true;
...
function Entity(id, mass, x, y, dx = 0, dy = 0, fixed = false, type = roid, typeFixed = false, isPrimary = false, primaryColor) {
	...
	this.periodicUpdate = function() {
		...
		// if (this.id == "Terra") {
		// 	console.log("Terra: " + this.x + "," + this.y);
		// 	console.log("Origin: " +originX + "," + originY);
		// }
	}
	...
	this.draw = function() {
		...
		if (
			(scaleX - this.radius * viewScale.val > innerWidth || 
			scaleY - this.radius * viewScale.val > innerHeight || 
			scaleX + this.radius * viewScale.val < 0 || 
			scaleY + this.radius * viewScale.val < 0) &&
			...
			) 
		{
			...
			if (!stopClear) {
				...
				if (showTrails && (this.mass >= trivialThreshold || showTrivial || this.id == target || this.isPrimary)) { //not combined below due to layering

					c.strokeStyle = this.trailColor;
					c.lineWidth = 1;

					c.beginPath();
					//c.moveTo(this.lastPos[0][0], this.lastPos[0][1])
					...
					c.lineTo(scaleX, scaleY)
					c.stroke();
					//console.log("stroke " + this.id);
					c.closePath();
				}
				...
				if (this.mass >= trivialThreshold || showTrivial || this.id == target || this.isPrimary) {
					...
					var tagX = scaleX + this.radius * viewScale.val + 4; //4
					var tagY = scaleY + this.radius * viewScale.val + 4; //2
